...
class TestMatrix:
    def test_extract_row_from_one_number_matrix(self) -> None:
        matrix = Matrix("1")
        assert matrix.row(1) == [1]

    def test_can_extract_row(self) -> None:
        matrix = Matrix("1 2\n3 4")
        assert matrix.row(2) == [3, 4]

    def test_extract_row_where_numbers_have_different_widths(self) -> None:
        matrix = Matrix("1 2\n10 20")
    ...
    def test_extract_column_from_one_number_matrix(self) -> None:
        matrix = Matrix("1")
        assert matrix.column(1) == [1]

    def test_can_extract_column(self) -> None:
        matrix = Matrix("1 2 3\n4 5 6\n7 8 9")
    ...
    def test_extract_column_where_numbers_have_different_widths(self) -> None:
